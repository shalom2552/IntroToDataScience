...
awk '{print tolower($0)}' Alice_book | tr -cd '\ \t\n[:alnum:]' > Alice_book_tolower.txt

# Cleaning begin trash
awk 'BEGIN{a=0}{if ($1 == "chapter"){ a=1}
	if (a == 1){print $0}}' Alice_book_tolower.txt > Alice_book_clean.txt
rm -f Alice_book_tolower.txt
...
# Finding the most common pair and the Most common first word in book
rm -f output.txt
touch output.txt
awk '{
	b[$1]++;
...
# Finding the most common pair and the Most common first word in each chapter
cd chapters
for n in {1..12};
do
	awk '{
...
# Number of lines shorter than the average
sed -i "s/chapter//g" Alice_book_clean.txt
awk 'BEGIN{sum=0}
	{if (NF <= 4) sum+=1}
	END{print "Number of lines shorter than the average:",sum
}' Alice_book_clean.txt >> output.txt

# alice average position in row
